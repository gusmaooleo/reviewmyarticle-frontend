...
export default function FileUploadCompact({
  ...
  return (
    <div className={cn('w-full', className)}>
      ...
      <div
        className={cn(
          'h-full flex items-center gap-3 rounded-lg border border-border border-dashed p-4 transition-colors',
      ...
      >
        ...
        {/* Upload Button */}
        <Button type='button' onClick={openFileDialog} size="sm" className={cn(isDragging && 'animate-bounce')}>
        ...
        <div className="flex flex-1 items-center gap-2">
          {files.length === 0 ? (
            <p className="text-sm text-muted-foreground">Submeta aqui o artigo no formato (PDF)</p>
          ) : (
            files.map((fileItem) => (
